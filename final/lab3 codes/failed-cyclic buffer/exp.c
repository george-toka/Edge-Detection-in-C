...
void applyPrewitt() {
    ...
    do{ 
        ...
        temp1 = start_idx + bbi + width;
        temp2 = start_idx + bbi - width;

        small_buff[0] = big_buff[(temp2 - 1) % bigbuffsize]; 	// I1
        small_buff[1] = big_buff[temp2 % bigbuffsize]; 		// I2
        small_buff[2] = big_buff[(temp2 + 1) % bigbuffsize];	// I3
        small_buff[3] = big_buff[(bbi - 1) % bigbuffsize];			// I4
        small_buff[4] = big_buff[bbi % bigbuffsize];				// I5
        small_buff[5] = big_buff[(bbi + 1) % bigbuffsize];			// I6
        small_buff[6] = big_buff[(temp1 - 1) % bigbuffsize];	// I7
        small_buff[7] = big_buff[temp1 % bigbuffsize];		// I8
        small_buff[8] = big_buff[(temp1 + 1) % bigbuffsize];	// I9
        small_buff[9] = big_buff[(temp2 + 2) % bigbuffsize];	// I10
        small_buff[10] = big_buff[(bbi + 2) % bigbuffsize];			// I11
        small_buff[11] = big_buff[(temp1 + 2) % bigbuffsize];	// I12
        ...
        out_buff[temp2-start_idx-1] = abs(P1) + abs(P2);
        out_buff[temp2-start_idx] = abs(P3) + abs(P4);
        
        if (out_buff[temp2-start_idx-1] > max_value) max_value = out_buff[temp2-start_idx-1];
        if (out_buff[temp2-start_idx] > max_value) max_value = out_buff[temp2-start_idx];
        ...
        if(idx % width == 1) {
            ...
            // Overwrite the oldest line with the new line from input_image
            for (i = 0; i < width; i++) {
                big_buff[(old_line_idx + i) % bigbuffsize] = input_image[t + i];
            }
            
            for (i = 0; i < width-2; i++){
                outim[width * (cache_load_rep - 2) + i + 1] = out_buff[i];
            }
            start_idx = (start_idx + width) % bigbuffsize;
            cache_load_rep += 1;
            bbi = width + 1;
            //printf("%d \n", start_idx);
        }
